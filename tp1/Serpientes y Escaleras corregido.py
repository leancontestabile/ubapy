...
def generador_casillero_banana(casilleros_ocupados: list) -> dict:
    """
    Precondicion: Debe tener una lista con todas las posiciones ocupadas por otros casilleros hasta el momento, estas deben ser enteros.
    Postcondicion: Realiza un diccionario con la posicion del casillero y a donde desplaza el mismo verificando que no se interpongan en casilleros ocupados previamente, de ser asi se escogera otro casillero.
    """
...
def generador_casillero_magico(casilleros_ocupados: list) -> dict:
    """
    Precondicion: Debe tener una lista con todas las posiciones ocupadas por otros casilleros hasta el momento, estas deben ser enteros.
    Postcondicion: Realiza un diccionario con la posicion del casillero, el mismo verificando que no se interponga en casilleros ocupados previamente, de ser asi escogera otro casillero aleatoriamente. Retornara el casillero ocupado con la value "magia".
    """
...
def generador_casillero_rushero(casilleros_ocupados: list) -> dict:
    """
    Precondicion: Debe tener una lista con todas las posiciones ocupadas por otros casilleros hasta el momento, estas deben ser enteros.
    Postcondicion: Realiza un diccionario con la posicion del casillero y a donde desplaza el mismo verificando que no se interpongan en casilleros ocupados previamente y sumando de 1 en 1 la value para conseguir que la misma se posicione en la esquina del tablero. 
    """
...
def generador_casillero_hongos(casilleros_ocupados: list) -> dict:
    """
    Precondicion: Debe tener una lista con todas las posiciones ocupadas por otros casilleros hasta el momento, estas deben ser enteros.
    Postcondicion: Realiza un diccionario con la posicion del casillero y a donde desplaza el mismo verificando que no se interpongan en casilleros ocupados previamente y restando de 1 en 1 la value para conseguir que la misma se posicione en la esquina del tablero, luego le suma 1.
    """
    validacion: bool = True
...
def movimiento_dado(posicion: int) -> list:
    """
    Precondicion: La posicion del jugador tiene que ser un entero
    Postcondicion: Se elige un entero del 1 al 6 aleatoriamente, le suma el numero a la posicion y arma una lista con el numero aleatorio y la nueva posicion del jugador.
    """
...
def contadores_casilleros(jugador1: list, jugador2: list, contadores: dict, casilleros_banana: dict, casilleros_magico: dict, casillero_rushero: dict, casillero_hongo: dict) -> dict:
    """
    Precondicion: Las posiciones de los jugadores tiene que ser entero. El diccionario contadores tiene que tener una key y una value para cada casillero que se quiere verificar.
    Postcondicion: Verifica la posicion de los jugadores, si estan posicionados en algun diccionario ingresado, suma +1 a su value respectiva en el diccionario de contadores.
    """
...
def movimiento(posicion_jugador: list, casilleros_banana: dict, casilleros_magico: dict, casillero_rushero: dict, casillero_hongos: dict) -> int:
    """
    Precondicion: La posicion del jugador tiene que ser un entero. Los casilleros tienen que tener un diccionario con una key y una value entero.
    Postcondicion: Verifica si la posicion del jugador es la key de algun casillero y lo desplaza a la value del mismo.
    """
    movimiento = posicion_jugador
...
def jugadores(jugador1: int, nombrej1: str, jugador2: int, nombrej2: str, casillero_bananas: dict, casillero_magicos: dict, casillero_rushers: dict, casillero_hongo: dict, tablero: str, random: int) -> list:
    if random == 1:
...
def cambio_tablero(posicion: int, tablero: str, jugador: str) -> str:
    """
    Precondicion: La posicion tiene que estar entre los numeros del tablero. Tablero a modificar, Indicador de que jugador esta siendo reemplazado en el tablero.
    Postcondicion: Verifica la posicion ingresada, si es menor a 10 se le agrega un 0 por delante o no y luego reemplaza la posicion por el indicador del jugador ingresado.
    """
    if posicion < 10:
